...
var marker = [];
var mapObj;
var mapAddrId,mapLngId,mapLatId;
function showAMap(parentPanelId,addrId,lngId,latId) {
	mapAddrId = addrId;
	mapLngId = lngId;
	mapLatId = latId;
	
	c = [];
	c.push('<div id="create_map" >');
	c.push('	<div id="mapContainer" style="height:400px;width:500px;" >');
	c.push('		<div id="tip">');
	c.push('			<b>请输入关键字：</b>');
	c.push('			<input type="text" id="keyword" name="keyword" value="" onkeydown="keydown(event)" style="width:192px" /> ');
	c.push('			<div id="result1" name="result1"></div>');
	c.push('		</div>');
	c.push('	</div>');
	c.push('</div>');

	$('#create_map').dialog('destroy');
	$(parentPanelId).append(c.join(""));
	$('#create_map').dialog({
		title : '定位地图',
		closed : false,
		width : 520,
...
function selectResult(index) {
	...
	var text = document.getElementById("divid" + (index + 1)).innerHTML
			.replace(/<[^>].*?>.*<\/[^>].*?>/g, "");
	var cityCode = document.getElementById("divid" + (index + 1)).getAttribute(
			'data');
...
function addmarker(i, d) {
	...
	var infoWindow = new AMap.InfoWindow({
		content : "<h3><font color=\"#00a6ac\">  " + (i + 1) + ". " + d.name
				+ "</font></h3>" + TipContents(d.type, d.address, d.tel),
	...
	// 单击div时，注入对应的位置信息到org地址标签
	var setOrgAddre = function(e) {
		var addr = mar.getTitle();
		var lng = mar.getPosition().lng;
		var lat = mar.getPosition().lat;

		var addrObj = $(mapAddrId);
		var lngObj = $(mapLngId);
		var latObj = $(mapLatId);
	...
	AMap.event.addListener(mar, "click", setOrgAddre);
}
